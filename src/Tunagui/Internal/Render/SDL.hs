...
--
-- runRender :: MonadIO m => SDL.Renderer -> RenderP m a -> m a
-- runRender = interpret
--   where
--     interpret :: MonadIO m => SDL.Renderer -> RenderP m a -> m a
--     interpret r is = eval r =<< viewT is
--
--     eval :: MonadIO m => SDL.Renderer -> ProgramViewT RenderI m a -> m a
...
--
--     -- Text
--     eval r (TextSize font text :>>= is) = do
--       rect <- liftIO $ do
--         (w, h) <- TTF.size font text
--         return $ T.S $ fromIntegral <$> V2 w h
--       interpret r (is rect)
--     eval r (RenderText font (T.P p) text :>>= is) = do
